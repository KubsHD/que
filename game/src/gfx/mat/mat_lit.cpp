...
void MAT_Lit::create(Renderer2* ren)
{
	PipelineBuilder pipelineBuilder;

	auto vs = PipelineBuilder::load_shader_module("shader/lit_pbr.vs_c", GfxDevice::device);
	auto ps = PipelineBuilder::load_shader_module("shader/lit_pbr.ps_c", GfxDevice::device);
...
void MAT_Lit::clear(VkDevice device)
{
	vkDestroyDescriptorSetLayout(device, this->material_layout, nullptr);
}

MaterialInstance MAT_Lit::write(VkDevice device, const Resoruces& res, DescriptorAllocator* allocator)
{
	...
	writer.write_image(0, res.diffuse.view, res.sampler, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER);
	writer.write_image(1, res.normal.view, res.sampler, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER);
	writer.write_image(2, res.orm.view, res.sampler, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER);
	writer.write_image(3, res.emission.view, res.sampler, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER);
