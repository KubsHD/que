...
namespace pipeline
{
	...
	{
		std::vector<char> vertexSource = Asset::Instance->read_all_bytes("shader/mesh.vert.spv");
		auto vs = gapi.CreateShader({ GraphicsAPI::ShaderCreateInfo::Type::VERTEX, vertexSource.data(), vertexSource.size() });

		std::vector<char> fragmentSource = Asset::Instance->read_all_bytes("shader/mesh.frag.spv");
