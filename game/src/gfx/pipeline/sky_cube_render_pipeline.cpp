...
namespace pipeline {
	...
	{
		std::vector<char> vertexSource = asset_mgr->read_all_bytes("data/shader/equi2cube.vert.spv");
		auto vs = gapi.CreateShader({ GraphicsAPI::ShaderCreateInfo::Type::VERTEX, vertexSource.data(), vertexSource.size() });

		std::vector<char> fragmentSource = asset_mgr->read_all_bytes("data/shader/equi2cube.frag.spv");
